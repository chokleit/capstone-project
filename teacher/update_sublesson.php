...
if(isset($_POST['update'])){
   ...
   $update_content = $conn->prepare("UPDATE `lesson` SET title = ?, description = ?, lesson = ?, content_number = ? WHERE id = ?");
   $update_content->execute([$title, $description, $lesson, $content_number, $video_id]);

   if(!empty($playlist)){
      $update_playlist = $conn->prepare("UPDATE `sublesson` SET lesson_id = ? WHERE id = ?");
   ...
   if(!empty($thumb)){
      if($thumb_size > 2000000){
         ...
         $update_thumb = $conn->prepare("UPDATE `sublesson` SET thumb = ? WHERE id = ?");
...
if (!empty($pdf)) {
    ...
    } elseif ($pdf_size > 5000000) { // 5MB file size limit
        $message[] = 'PDF file size is too large!';
    } else {
        // Update the PDF file in the database
        $update_pdf = $conn->prepare("UPDATE `sublesson` SET pdf = ? WHERE id = ?");
        $update_pdf->execute([$rename_pdf, $video_id]);
        
        // Move the uploaded PDF file to the server
        move_uploaded_file($pdf_tmp_name, $pdf_folder);
        
        // Remove the old PDF file if it exists and is different from the new one
        if (!empty($old_pdf) && $old_pdf != $rename_pdf) {
...
if(isset($_POST['delete_video'])){
   ...
   $delete_video_thumb = $conn->prepare("SELECT thumb FROM `sublesson` WHERE id = ? LIMIT 1");
...
<section class="video-form">
   ...
   <?php
      $select_videos = $conn->prepare("SELECT * FROM `sublesson` WHERE id = ? AND tutor_id = ?");
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <p>update content number<span>*</span></p>
      <input type="text" name="title" maxlength="100" required placeholder="enter content number" class="box" value="<?= $fecth_videos['content_number']; ?>">
