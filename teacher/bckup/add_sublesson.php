...
if(isset($_POST['submit'])){
   ...
   $pdf = filter_var($_FILES['pdf']['name'], FILTER_SANITIZE_STRING);
   ...
   if ($_FILES['pdf']['size'] > 5000000) { // 5MB limit
      $message[] = 'PDF file size is too large!';
   } elseif (!in_array(strtolower($pdf_ext), $allowed_pdf_types)) {
      $message[] = 'Invalid PDF file type!';
   } else {
      // Process uploads
      if ($thumb_size > 2000000) { // Thumbnail size validation
         $message[] = 'Thumbnail image size is too large!';
      } else {
         ...
         $message[] = 'New course uploaded with PDF!';
      }
   }
}
